...
int RW_Thread_Mutex::init()
{
	...
	*/
	...
	if(nRet != 0)
	{
		CB_LOG_ERROR("pthread_rwlock_init failed. ret:%d, errno:%d, errmsg:%s\n", nRet, errno, strerror(errno));	
	}
	else
	{
		_init = true;
...
int RW_Thread_Mutex::unlock()
{
    ...
    if (nRet != 0)
    {
    	CB_LOG_ERROR("pthread_rwlock_unlock failed. ret:%d, errno:%d, errmsg:%s\n", 
			nRet, errno, strerror(errno));	
...
int RW_Thread_Mutex::lock_read()
{
    ...
    if (nRet != 0)
    {
    	CB_LOG_ERROR("pthread_rwlock_rdlock failed. ret:%d, errno:%d, errmsg:%s\n", 
			nRet, errno, strerror(errno));	
    }
	else
	{
...
int RW_Thread_Mutex::lock_write()
{
    ...
    if (nRet != 0)
    {
    	CB_LOG_ERROR("pthread_rwlock_wrlock failed. ret:%d, errno:%d, errmsg:%s\n", 
			nRet, errno, strerror(errno));	
    }
	else
	{
...
int RW_Thread_Mutex::destroy()
{
	...
	if(!_destroy)
	{
		...
		if(nRet == 0)
		{
			CB_LOG_DEBUG("pthread_rwlock_destroy success.\n");
			_destroy = true;
		}
		else if(nRet == EBUSY)
		{
			CB_LOG_WARN("the pthread rw mutex is locked now, can't destroy.\n");
		}
		else
		{
			CB_LOG_ERROR("pthread_rwlock_destroy failed. ret:%d, errno:%d, errmsg:%s\n", 
				nRet, errno, strerror(errno));
		}
