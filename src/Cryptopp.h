#ifndef __CPPBRICK_CRYPTOPP_H__
#define __CPPBRICK_CRYPTOPP_H__
...
public:
	...
	*/
	static bool md5_calculate_digest(const std::string src, std::string &digest);	
	...
	后面的数字表示生成的摘要长度
	*/
	static bool sha256_calculate_digest(const std::string src, std::string &digest);
	static bool sha1_calculate_digest(const std::string src, std::string &digest);
	static bool hmacsha1_calculate_digest(const std::string key, const std::string src, std::string &digest, bool uppercase = false);
};
...
public:
	...
	static bool base64_encrypt(const char *pSrc, const unsigned int len, char* &pDst);

	static bool base64_decrypt(const char *pSrc, char* &pDst, unsigned int &len);

	/*
	AES 可以使用128 bit (16 byte，缺省)、192 bit (24 byte)、256 bit (32 byte) 密钥
	只能加密128 bit (16 byte)的数据块， 加密后的数据块长度也是128位
	*/
	static bool aes_cbc_encrypt(const std::string key, const std::string plaintext, std::string &ciphertext, const std::string iv="");
	
	static bool aes_cbc_decrypt(const std::string key, const std::string ciphertext, std::string &plaintext, const std::string iv="");
	
	/*
	DES 使用64 bit (8 byte)密钥加密64bit (8byte)的数据块， 加密后的数据块长度也是64位
	*/	
	static bool des_cbc_encrypt(const std::string key, const std::string plaintext, std::string &ciphertext, const std::string iv="");

	static bool des_cbc_decrypt(const std::string key, const std::string ciphertext, std::string &plaintext, const std::string iv="");		
};
